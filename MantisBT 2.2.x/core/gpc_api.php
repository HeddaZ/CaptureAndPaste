...
function gpc_get_file( $p_var_name, $p_default = null ) {
    ...
    #############################################
    # To accept the direct-binary-data uploading.
    # @Hedda
    #############################################
        ...
        $first_file = $_POST [$p_var_name] [0];
        if (substr($first_file, 0, strlen($data_prefix)) == $data_prefix) {
            $data = base64_decode(substr($first_file, strlen($data_prefix)));

            # Manually write file to temp directory, but it's not compatible with move_uploaded_file method.
            $tmp_name = tempnam('/tmp', $file_ext);
            file_put_contents($tmp_name, $data);
            chmod($tmp_name, 0777);
            $file_name = pathinfo($tmp_name) ['filename'] . '.' . $file_ext;

            # RENAME token
            session_set('rename_token', md5($file_type . $tmp_name . $file_name));

            $t_result = array();
            $t_result['name'][0] = $file_name;
            $t_result['type'][0] = $file_type;
            $t_result['size'][0] = strlen($data);
            $t_result['tmp_name'][0] = $tmp_name;
            $t_result['error'][0] = 0;
        }
    #############################################
